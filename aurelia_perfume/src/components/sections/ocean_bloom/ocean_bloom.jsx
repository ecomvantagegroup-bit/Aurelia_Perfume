...
export default defineComponent({
  ...
  setup() {
    ...
    const fragranceNotes = [
      {
        category: 'TOP NOTE',
        detail: 'Crisp Sea Salt & Bergamot',
        // Pyramid apex on mobile (spans 2 columns centered), 1st column on desktop
        responsiveClass: 'col-span-2 md:col-span-2 lg:col-span-1 order-1',
      },
      {
        category: 'HEART NOTE',
        detail: 'Ocean Water Lily & Sage',
        // Pyramid bottom-left on mobile, 2nd column on desktop
        responsiveClass: 'col-span-1 md:col-span-1 lg:col-span-1 order-2',
      },
      {
        category: 'BASE NOTE',
        detail: 'Driftwood & Ambergris',
        // Pyramid bottom-right on mobile, 3rd column on desktop
    ...
    return () => (
      <section
        ref={sectionRef}
        id="sec-ocean"
        class="ocean-bloom-section relative min-h-screen w-full flex flex-col justify-between p-6 md:p-12 lg:p-24 bg-transparent select-none overflow-hidden"
      >
        ...
        <div ref={contentRef} class="my-auto z-10 max-w-2xl ocean-reveal space-y-4 md:space-y-6 py-6">
          <h1 class="text-5xl sm:text-6xl md:text-8xl font-extralight tracking-tight text-white font-serif leading-none drop-shadow-md">
            Ocean <br />
            <span class="italic font-normal bg-gradient-to-r from-indigo-200 via-indigo-300 to-cyan-400 bg-clip-text text-transparent">
              Bloom
            </span>
          </h1>
          <p class="text-xs sm:text-sm md:text-base text-slate-200/90 leading-relaxed font-light max-w-md backdrop-blur-[2px] bg-slate-950/20 p-4 rounded-xl border border-indigo-500/10">
        ...
        {/* Responsive Fragrance Note Cards Layout */}
        <div
          ref={nodesRef}
          class="z-10 grid grid-cols-2 lg:grid-cols-3 gap-3 md:gap-4 pt-6 border-t border-indigo-500/20 pointer-events-auto"
        >
          {fragranceNotes.map((note) => (
            <div
              key={note.category}
              class={`ocean-node p-4 md:p-5 rounded-2xl border border-indigo-400/20 bg-slate-950/80 backdrop-blur-2xl shadow-2xl transition-all duration-300 hover:border-indigo-400/60 hover:bg-indigo-950/60 hover:shadow-[0_0_25px_rgba(99,102,241,0.2)] ${note.responsiveClass}`}
            >
